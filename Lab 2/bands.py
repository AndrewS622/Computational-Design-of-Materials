...
! See
files
INPUT_BANDS. * in Doc / directory
for usage
    !
    !
    USE
    io_files, ONLY: prefix, tmp_dir
    USE
    mp_global, ONLY: npool, nproc_pool, nproc_file, &
    nproc_pool_file, mp_startup
    USE
    control_flags, ONLY: twfcollect, gamma_only
    USE
    environment, ONLY: environment_start, environment_end
    USE
    wvfct, ONLY: nbnd
    USE
    klist, ONLY: nkstot, two_fermi_energies
    USE
    noncollin_module, ONLY: noncolin, i_cons
    USE
    lsda_mod, ONLY: nspin
    USE
    io_global, ONLY: ionode, ionode_id, stdout
    USE
    mp, ONLY: mp_bcast
    USE
    ...
    # endif
    CALL
    environment_start('BANDS')
    !
    !   set
    default
    values
    for variables in namelist
        !
        prefix = 'pwscf'
        CALL
        get_environment_variable('ESPRESSO_TMPDIR', outdir)
        IF(trim(outdir) == ' ')
        outdir = './'
        filband = 'bands.out'
        lsym =.true.
        ...
        !
        CALL
        mp_bcast(ios, ionode_id, world_comm)
        IF(ios /= 0) CALL
        errore('bands', 'reading bands namelist', abs(ios))
        !
        ! ...
        Broadcast
        variables
        !
        CALL
        mp_bcast(tmp_dir, ionode_id, world_comm)
        CALL
        mp_bcast(prefix, ionode_id, world_comm)
        CALL
        mp_bcast(filband, ionode_id, world_comm)
        CALL
        mp_bcast(filp, ionode_id, world_comm)
        CALL
        mp_bcast(spin_component, ionode_id, world_comm)
        CALL
        mp_bcast(firstk, ionode_id, world_comm)
        CALL
        mp_bcast(lastk, ionode_id, world_comm)
        CALL
        mp_bcast(lp, ionode_id, world_comm)
        CALL
        mp_bcast(lsym, ionode_id, world_comm)
        CALL
        mp_bcast(lsigma, ionode_id, world_comm)
        CALL
        mp_bcast(no_overlap, ionode_id, world_comm)
        CALL
        mp_bcast(plot_2d, ionode_id, world_comm)
        ...
        IF(npool > 1. and..
        not.(lsym. or.no_overlap)) CALL
        errore('bands', &
        'pools not implemented', npool)
        IF(spin_component < 1.OR.spin_component > 2) &
        CALL
        errore('bands', 'incorrect spin_component', 1)
        !
        !   Now
        allocate
        space
        for pwscf variables, read and check them.
        !
        CALL
        read_file()
        !
        IF(gamma_only)
        CALL
        errore('bands', 'gamma_only case not implemented', 1)
        ...
        CALL
        errore('punch_band', 'lsigma requires noncollinear run', 1)
        IF(spin_component /= 1. and.nspin /= 2 ) &
        CALL
        errore('punch_bands', 'incorrect spin_component', 1)
        !
        CALL
        openfil_pp()
        !
        ...
        THEN
        CALL
        punch_band_2d(filband, spin_component)
        ELSE
        CALL
        punch_band(filband, spin_component, lsigma, no_overlap)
        IF(lsym)
        CALL
        ...
        kinds, ONLY: dp
        USE
        ions_base, ONLY: nat, ityp, ntyp = > nsp
        USE
        cell_base, ONLY: at
        USE
        constants, ONLY: rytoev
        USE
        gvect, ONLY: g, ngm
        USE
        klist, ONLY: xk, nks, nkstot, ngk, igk_k
        USE
        io_files, ONLY: iunpun, nwordwfc, iunwfc
        USE
        wvfct, ONLY: nbnd, et, npwx
        USE
        uspp, ONLY: nkb, vkb, qq
        USE
        uspp_param, ONLY: upf, nh, nhm
        USE
        noncollin_module, ONLY: noncolin, npol
        USE
        wavefunctions_module, ONLY: evc
        USE
        io_global, ONLY: ionode, ionode_id, stdout
        USE
        mp, ONLY: mp_bcast
        ...
        mp_world, ONLY: world_comm
        USE
        becmod, ONLY: calbec, bec_type, allocate_bec_type, &
        deallocate_bec_type
        ...
        INTEGER, ALLOCATABLE:: closest_band(:,:)! index
        for band ordering
            REAL(DP), ALLOCATABLE:: sigma_avg(:,:,:) ! expectation
            value
            of
            sigma
        REAL(DP), ALLOCATABLE:: et_(:,:) ! reordered
        eigenvalues in eV
        ...
        IF(ios(0) /= 0 ) &
        CALL
        errore('punch_band', 'Opening filband file', abs(ios(0)))
        DO
        ipol = 1, 4
        IF(ios(ipol) /= 0 ) &
        CALL
        errore('punch_band', 'Opening filband.N file ', ipol)
        ENDDO
        !
        CALL
        find_nks1nks2(1, nkstot, nks1tot, nks1, nks2tot, nks2, spin_component)
        !
        ! index
        of
        largest
        ...
        CALL
        allocate_bec_type(nkb, nbnd, becp)
        !
        DO
        ik = nks1, nks2
        !
        ...
        !
        CALL
        davcio(evc, 2 * nwordwfc, iunwfc, ik, - 1)
        !
        ! calculate
        becp = < psi | beta >, needed
        to
        compute
        spsi = S | psi >
        !
        npw = ngk(ik)
        CALL
        init_us_2(npw, igk_k(1, ik), xk(1, ik), vkb)
        CALL
        ...
        average
        magnetization
        for noncolinear case
            !
            IF(noncolin) &
            CALL
            compute_sigma_avg(sigma_avg(1, 1, ik), becp % nc, ik, lsigma)
            !
            IF(ik > nks1.AND..NOT.no_overlap)
            THEN
            !
            ! compute
            correspondence
            ...
            ! for unassigned bands...
                ...
                IF(noncolin)
                CALL
                poolrecover(sigma_avg, 4 * nbnd, nkstot, nks)
                !
                CALL
                ...
                !
                CALL
                punch_plottable_bands(filband, nks1tot, nks2tot, nkstot, nbnd, &
                xk, et_ )
                !
                DO
                ik = nks1tot, nks2tot
                ...
                ipol = 1, 4
                IF(lsigma(ipol))
                WRITE(iunpun_sigma(ipol), &
                '(" &plot nbnd=",i4,", nks=",i6," /")') &
                nbnd, nks2tot - nks1tot + 1
                ENDDO
                ENDIF
                WRITE(iunpun, '(10x,3f10.6)')
                ...
                a
                file
                for each band and writes on output
                !  kx, ky, energy,
                !  kx, ky, energy
                !.., .., ..
                !  where
                ...
                kinds, ONLY: DP
                USE
                constants, ONLY: eps8, rytoev
                USE
                lsda_mod, ONLY: nspin
                USE
                klist, ONLY: xk, nkstot, nks
                USE
                wvfct, ONLY: et, nbnd
                USE
                io_files, ONLY: iuntmp
                USE
                ...
                CALL
                poolcollect(3, nks, xk, nkstot, xk_collect)
                CALL
                poolcollect(nbnd, nks, et, nkstot, et_collect)
                ...
                IF(ABS(xk_collect(1, j) - xkdum(1)) > eps8.OR. &
                   ...
                   ABS(xk_collect(3, j) - xkdum(3)) > eps8)
                THEN
                n2 = j - 1
                ...
                loop_k
                n1 = nks_eff / n2
                IF(n1 * n2 /= nks_eff) CALL
                errore('punch_band_2d', &
                ...
                100
                CALL
                errore('punch_band_2d', 'Problem opening outputfile', ios)
                ijk = 0
                DO
                i1 = 1, n1
                DO
                ...
                !
                !
                IF(nks1tot < 1.OR.nks2tot > nkstot.OR.nkstot < 1.OR.nbnd < 1)
                THEN
                CALL
                infomsg('punch_plottable_bands', 'incorrect input data, exiting')
                RETURN
                END
                IF
                ios = 0
                ...
                DO
                n = nks1tot, nks2tot
                IF(n == nks1tot.OR.n == nks2tot)
                THEN
                high_symmetry(n) =.true.
                    ELSE
                k1(:) = xk(:, n) - xk(:, n - 1)
                k2(:) = xk(:, n + 1) - xk(:, n)
                IF(k1(1) * k1(1) + k1(2) * k1(2) + k1(3) * k1(3) < 1.0
                d - 8.OR. &
                k2(1) * k2(1) + k2(2) * k2(2) + k2(3) * k2(3) < 1.0
                d - 8 ) THEN
                CALL
                infomsg('punch_plottable_bands', 'two consecutive same k, exiting')
                RETURN
                END
                IF
                ...
                point
                !
                IF(xk(1, n) ** 2 + xk(2, n) ** 2 + xk(3, n) ** 2 < 1.0
                d - 8) high_symmetry(n) =.true.
                !
                !   save
                the
                typical
                ...
                !
                IF(n == nks1tot + 1)
                dxmod_save = sqrt(k1(1) ** 2 + k1(2) ** 2 + k1(3) ** 2)
                ENDIF
                ENDDO
                ...
                THEN
                CALL
                infomsg('punch_plottable_bands', 'too many lines, exiting')
                RETURN
                END
                IF
                npoints(nlines) = 1
